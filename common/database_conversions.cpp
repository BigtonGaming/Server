	...
	if (runconvert == 1) {
		...
		for (auto row = results.begin(); row != results.end(); ++row) {
			...
			if (runconvert == 1) {
				...
				if (pp->tribute_time_remaining < 0 ||
					pp->tribute_time_remaining == 4294967295) { pp->tribute_time_remaining = 0; }
		...
		if (runconvert == 1) {
			std::string rquery = StringFormat("RENAME TABLE `character_` TO `character_old`");
...
bool Database::CheckDatabaseConvertCorpseDeblob()
{
	...
	if (results.RowCount() != 0) {
		...
		results = QueryDatabase(rquery);
		for (auto row = results.begin(); row != results.end(); ++row) {
			std::string squery   = StringFormat(
				"SELECT id, charname, data, time_of_death, is_rezzed FROM character_corpses WHERE `charid` = %i",
				atoi(row[0]));
			auto        results2 = QueryDatabase(squery);
			for (auto   row2     = results2.begin(); row2 != results2.end(); ++row2) {
				...
				/* SoF+ */
				uint32 esize1 = (sizeof(Convert::DBPlayerCorpse_Struct_temp) +
								 (dbpc->itemcount * sizeof(Convert::player_lootitem_temp::ServerLootItem_Struct_temp)));
				uint32 esize2 = (sizeof(Convert::classic_db_temp::DBPlayerCorpse_Struct_temp) + (dbpc_c->itemcount *
																								 sizeof(Convert::player_lootitem_temp::ServerLootItem_Struct_temp)));
				...
				if (is_sof) {
					scquery = StringFormat(
						"UPDATE `character_corpses` SET \n"
						"`is_locked` =          %d,\n"
						"`exp` =                 %u,\n"
						"`size` =               %f,\n"
						"`level` =              %u,\n"
					...
					for (unsigned int i = 0; i < dbpc->itemcount; i++) {
						if (first_entry != 1) {
							scquery     = StringFormat(
								"REPLACE INTO `character_corpse_items` \n"
